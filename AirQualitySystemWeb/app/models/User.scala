...
case class User (email: String, password: String, fullName: String, status: String)
...
class UserTableModel(tag: Tag) extends Table[User](tag, "USERS") {
  ...
  override def * =
    (email, password, fullName, status) <>(User.tupled, User.unapply)
}
...
class UsersModel @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)(implicit executionContext: ExecutionContext) extends HasDatabaseConfigProvider[JdbcProfile] {
